...
func (v *validatorRG) getBalance(msg tgbotapi.MessageConfig) {
	target, _ := botExecCmdOut("celocli account:balance $CELO_VALIDATOR_RG_ADDRESS", msg)
	target1, _ := botExecCmdOut("celocli lockedgold:show $CELO_VALIDATOR_RG_ADDRESS", msg)
	gold := cmd.ParseAmount(target, "gold")
	goldVal := isZero(gold, "goldVal")
	usd := cmd.ParseAmount(target, "usd")
	usdVal := isZero(usd, "usdVal")
...
func (v *validatorGrRG) getBalance(msg tgbotapi.MessageConfig) {
	target, _ := botExecCmdOut("celocli account:balance $CELO_VALIDATOR_GROUP_RG_ADDRESS", msg)
	target1, _ := botExecCmdOut("celocli lockedgold:show $CELO_VALIDATOR_GROUP_RG_ADDRESS", msg)
	gold := cmd.ParseAmount(target, "gold")
	goldVal := isZero(gold, "goldVal")
	usd := cmd.ParseAmount(target, "usd")
	usdVal := isZero(usd, "usdVal")
...
func UpdateBalance(acc accountManager, msg tgbotapi.MessageConfig) {
	acc.getBalance(msg)
}
