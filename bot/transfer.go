...
func (v *validatorBf) transfer(bot *tgbotapi.BotAPI, msg tgbotapi.MessageConfig) {
	msg.ParseMode = "Markdown"

	botSendMsg(bot, msg, boldText("Transferring gold from validator releaseGold beneficiary address was requested"))
	goldAvailable, _ := decimal.NewFromString(v.usd)
	msg.Text = v.xferGold(bot, msg, goldAvailable)
	botSendMsg(bot, msg, msg.Text)
}

func (v *validatorGrBf) transfer(bot *tgbotapi.BotAPI, msg tgbotapi.MessageConfig) {
	msg.ParseMode = "Markdown"
...
func (v *validatorBf) xferGold(bot *tgbotapi.BotAPI, msg tgbotapi.MessageConfig, amount decimal.Decimal) string {
	zeroValue, _ := decimal.NewFromString("0")
	if amount.Cmp(zeroValue) == 1 {
		toXfer := amount.String()
...
func (v *validatorGrBf) xferGold(bot *tgbotapi.BotAPI, msg tgbotapi.MessageConfig, amount decimal.Decimal) string {
	zeroValue, _ := decimal.NewFromString("0")
	if amount.Cmp(zeroValue) == 1 {
		toXfer := amount.String()
